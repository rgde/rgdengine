...
bool Sound::FillBuffer(LPDIRECTSOUNDBUFFER pDSB, DWORD startIndex, DWORD amount, DWORD *pAmtRead)
{
	HRESULT hr; 
	...
	if (FAILED(hr = m_pWaveFile->Read((BYTE *)pDSLockedBuffer, m_pos, dwDSLockedBufferSize, &dwWavDataRead)))
	{
	...
	if (dwWavDataRead < amount)
	{
		...
		FillMemory((BYTE *)pDSLockedBuffer + dwWavDataRead, dwDSLockedBufferSize - dwWavDataRead - 1, (BYTE)(m_pWaveFile->m_pwfx->wBitsPerSample == 8 ? 128 : 0));
	}
