...
void MarkupBase::parseBBCodes()
{
	...
	const TextViewVector& v = m_parser.getFormattedOutput();
	TextViewVector::const_iterator i = v.begin();
	TextViewVector::const_iterator end = v.end();
	while(i != end)
	{
		const TextView* tv = (*i).get();
		size_t pos = tv->start;
		size_t len = tv->len;
		size_t last = pos + len;
